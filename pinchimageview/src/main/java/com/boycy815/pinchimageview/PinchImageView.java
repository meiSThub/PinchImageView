...
public class PinchImageView extends ImageView {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            if (mPinchMode == PINCH_MODE_SCALE) {
                scaleEnd(); // 缩放之后，抬起双指，恢复最大或者最小缩放比例
            }
            mPinchMode = PINCH_MODE_FREE;
    ...
    private void scaleEnd() {
        ...
        Log.i(TAG, "scaleEnd: outerScale=" + outerScale + ",scalePost=" + scalePost);
        // 如果缩放修正后整体导致第二层缩放小于1（就是图片比fit center状态还小），重新修正缩放
        // if (outerScale * scalePost < 1f) {
        if (outerScale * scalePost < MIN_SCALE) {
            scalePost = MIN_SCALE / outerScale;// 从当前缩放比例 到 最小最所比例，需要放大的比例
        }
        Log.i(TAG, "scaleEnd: scalePost=" + scalePost);
        // 如果缩放修正不为1，说明进行了修正
        if (scalePost != 1f) {
            change = true;
        }
